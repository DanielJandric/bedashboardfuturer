...
const calculateSimulatedValue = (
    ...
    // const originalExitCap = property.tauxcapitalisationexit; // Removed unused variable

    if (currentRent === null || currentRent === undefined) return property.valeurcbrechf; // Return original if no rent
    if (exitCapRate <= 0) return 0; // Avoid division by zero

    // Very simplified example: Assume rent grows for 1 year then apply exit cap
    // A real model would be much more complex (multi-year projection, discounting, etc.)
    const futureRent = currentRent * (1 + rentalGrowth / 100);
    const simulatedValue = futureRent / (exitCapRate / 100);

    // Return the simulated value, or original if calculation fails
...
const SimulationSection: React.FC<SimulationSectionProps> = ({ properties, originalTotalValue }) => {
  ...
  return (
    <section className="bg-white rounded-lg shadow-md p-4 mb-8">
      <h2 className="text-xl font-semibold text-be_capital_dark_grey mb-4">Valuation Simulation</h2>
